...
class TestKickOutMemberViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.ownership.save()
        self.url = reverse('kickout_member', kwargs={
            'club_id': f'{self.club.id}',
            'user_id': f'{self.member.id}'
        })

    def test_kickout_member_url(self):
        self.assertEqual(self.url, f'/kickout_member/{self.club.id}/{self.member.id}')

    def test_successful_kickout_member(self):
    ...
    def test_successful_kickout_officer(self):
        self.membership.user_type = UserTypes.OFFICER
        self.membership.save()
        self.client.login(username=self.owner.username, password='Password123')
        ...
        self.assertEqual(officer_count_before, officer_count_after+1)
        redirect_url = reverse('club_members', kwargs={'club_id':f'{self.club.id}'})
        self.assertRedirects(response, redirect_url, status_code=302, target_status_code=200)
        self.assertTemplateUsed(response, 'club_member_list.html')

    def test_get_kickout_member_without_login(self):
        member_count_before = Member.objects.filter(user_type=UserTypes.MEMBER).count()
        response = self.client.get(self.url, follow=True)
        member_count_after = Member.objects.filter(user_type=UserTypes.MEMBER).count()
    ...
    def test_kickout_member_without_owner_permission_fails(self):
